...
def add_features(df, le):
    ...
    print(
        f"[피처 생성] 낙찰율: {df['낙찰율'].min():.3f} ~ {df['낙찰율'].max():.3f} / 평균: {df['낙찰율'].mean():.3f}"
    )
    print(
...
def run_cv(X, y):
    ...
    for rank, (name, res) in enumerate(
        sorted(cv_results.items(), key=lambda x: x[1]["mae"]), 1
    ):
        mae_원 = res["mae"] / 100 * 5e8
        print(
            f"  {rank}위 {name}: MAE {res['mae']:.4f}% ({mae_원 / 1e4:.0f}만원) / RMSE {res['rmse']:.4f}%"
        )
...
def main():
    ...
    print("\n" + "=" * 60)
    print("  1단계 완료")
    print(f"  이전 베이스라인 (참여업체수 포함 7개): 0.6200%  (5억 기준 310만원)")
    print("=" * 60)
    ...
    print("  최종 요약")
    print("=" * 60)
    print(f"  이전 베이스라인  : 0.6200%  (5억 기준 310만원)")
    print(
        f"  XGBoost 기본값   : {baseline:.4f}%  (5억 기준 {baseline / 100 * 5e8 / 1e4:.0f}만원)"
    )
    print(
        f"  XGBoost 튜닝 후  : {best_mae:.4f}%  (5억 기준 {best_mae / 100 * 5e8 / 1e4:.0f}만원)"
    )
